...
- [x] Unmount inactive tabs for heavy apps (especially Browser)
  ...
  - Where: `apps/viewer/src/components/Window/Window.tsx` (currently hides inactive tabs with `display: none`).
...
- [x] Guard Browser global keyboard listeners with `isActive`
  ...
  - Where: `apps/viewer/src/apps/browser/components/AddressBar.tsx` (currently always registers `window.addEventListener('keydown', ...)`).
...
- [ ] Add “Clear browser data/cache” and optional “Incognito” partition
  ...
  - Where: Browser webview uses `partition="persist:browser"` in `apps/viewer/src/apps/browser/components/WebViewContainer.tsx`.
...
- [x] Reduce Browser state update frequency (throttle noisy events)
  ...
  - Where: `apps/viewer/src/apps/browser/components/WebViewContainer.tsx` → calls `onStateChange(...)` on navigation/loading events.
...
- [x] Lazy-load apps instead of eager-loading all app modules at startup
  ...
  - Where: `apps/viewer/src/apps/index.ts` currently uses `import.meta.glob(..., { eager: true })`.
...
- [x] Replace `useWorkspaceStore()` “whole store” subscriptions with selectors in hot components
  ...
  - Where: Examples include `apps/viewer/src/App.tsx`, `apps/viewer/src/components/Desktop/Desktop.tsx`, `apps/viewer/src/components/Window/TabBar.tsx`.
...
- [x] Remove debug logging and expensive dev-only work in drag/tab code
  ...
  - Where: `apps/viewer/src/components/Window/TabBar.tsx` uses `console.group/log` during drag end.
...
- [x] Avoid work on every render that scales with number of windows/tabs
  ...
  - Where: `apps/viewer/src/components/Desktop/Desktop.tsx` recalculates focused/adjacent windows and scans open tabs.
...
- [ ] Add a “Performance Mode” toggle that reduces blur/glow/shadows
  ...
  - Where: `.holo-panel` uses `backdrop-filter: blur(12px)` in `apps/viewer/src/index.css`.
...
- [ ] Reduce file tree refresh/work on workspace switch (incremental updates)
  ...
  - Where: `apps/viewer/src/components/Desktop/Desktop.tsx` calls `refreshTree()` on workspace change.
