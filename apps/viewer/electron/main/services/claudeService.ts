 ...
 * Routes queries to the RAVEN Laptop Agent when available,
 * with fallback to direct Claude Agent SDK.
 */
...
// Laptop Agent configuration for RAVEN-MKII
const LAPTOP_AGENT_BASE = 'http://localhost:8423';
const LAPTOP_AGENT_HEALTH_TIMEOUT = 500;
...
export class ClaudeService {
  ...
  async query(options: ClaudeQueryOptions): Promise<void> {
    ...
    const laptopAgentAvailable = await this.checkLaptopAgent();

    if (laptopAgentAvailable) {
      console.log('[ClaudeService] Routing query to Laptop Agent');
      // Emit routing indicator so UI knows we're talking to RAVEN
      options.onMessage({ type: 'routing', agent: 'raven' } as unknown as SDKMessage);
      return this.queryLaptopAgent(options);
    }

    console.log('[ClaudeService] Laptop Agent unavailable, using direct SDK');
    // Emit routing indicator so UI knows we're talking directly to Claude
    options.onMessage({ type: 'routing', agent: 'claude' } as unknown as SDKMessage);
  ...
  private async checkLaptopAgent(): Promise<boolean> {
    try {
      const controller = new AbortController();
      const timeout = setTimeout(
        () => controller.abort(),
        LAPTOP_AGENT_HEALTH_TIMEOUT,
      );

      const res = await fetch(`${LAPTOP_AGENT_BASE}/health`, {
  ...
  private async queryLaptopAgent(options: ClaudeQueryOptions): Promise<void> {
    let accumulatedText = '';

    try {
      const res = await fetch(`${LAPTOP_AGENT_BASE}/query`, {
        method: 'POST',
      ...
      if (!res.ok) {
        throw new Error(
          `Laptop Agent returned ${res.status}: ${res.statusText}`,
        );
      }

      if (!res.body) {
        throw new Error('Laptop Agent response has no body');
      }
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (event.type === 'text' && event.content) {
            ...
            options.onMessage(
              this.buildAssistantMessage(accumulatedText, null),
            );
          } else if (event.type === 'done') {
            options.onMessage(
              this.buildAssistantMessage(accumulatedText, 'end_turn'),
            );
            options.onComplete();
            return;
          }
      ...
      // Stream ended without an explicit done event — complete anyway
      if (accumulatedText) {
        options.onMessage(
          this.buildAssistantMessage(accumulatedText, 'end_turn'),
        );
